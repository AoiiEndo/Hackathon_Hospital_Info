...
function MyComponent() {
  ...
  async function searchHospital() {
    const apiKey = '---'; // Google Custom Search JSON APIのAPIキー
    // 検索キーワードを組み立てる処理
    const keyword = `${address} ${searchInput} 病院`;

    // Google Custom Search JSON APIのURLを構築
    const apiUrl = `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=---&q=${encodeURIComponent(keyword)}&num=5`;
